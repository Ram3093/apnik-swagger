...
router.post('/:uid', validator(schema.create), (req, res) => {
	const suid = req.params.uid;
	controller
		.create(req.body, suid)
		.then((uid) => responseHandler.create(res, uid, 'SubSection'))
...
router.get('/', (req, res) => {
	controller
		.getAll()
		.then((data) => responseHandler.get(res, data, 'SubSection'))
		.catch((err) => responseHandler.err400(res, err));
});
...
router.patch('/:uid', validator(schema.patch), (req, res) => {
	...
	controller
		...
		.then(() => responseHandler.update(res, 'SubSection'))
		.catch((err) => responseHandler.err400(res, err));
});
...
router.patch('/:uid/publish', validator(schema.uid), (req, res) => {
	...
	controller
		.publish(uid)
		.then(() => responseHandler.publish(res, 'SubSection'))
		.catch((err) => responseHandler.err400(res, err));
});
...
router.patch('/:uid/unpublish', validator(schema.uid), (req, res) => {
	...
	controller
		.unpublish(uid)
		.then(() => responseHandler.unpublish(res, 'SubSection'))
		.catch((err) => responseHandler.err400(res, err));
});
